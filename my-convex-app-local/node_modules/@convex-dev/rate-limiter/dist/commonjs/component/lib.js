...
import { calculateRateLimit, fixedWindowValidator, getValueReturns, rateLimitArgs, configValidator, rateLimitReturns, tokenBucketValidator, } from "../shared.js";
import { checkRateLimitOrThrow, configWithDefaults, getShard, } from "./internal.js";
...
export const getValue = query({
    ...
    handler: async (ctx, args) => {
        ...
        const allShards = (await Promise.all(selectedShards.map((shard) => getShard(ctx.db, args.name, args.key, shard)))).map((state, i) => state ?? { value: config.capacity, ts: 0, shard: selectedShards[i] });
        ...
        const maxShard = values.reduce((a, b) => a.maxTs.value > b.maxTs.value ? a : b);
